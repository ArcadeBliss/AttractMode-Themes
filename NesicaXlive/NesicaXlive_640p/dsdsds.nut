...
/// [ Feature ] new text animation for title
...
////////////////////////////////
	class Grid extends Conveyor
	{
		...
		constructor()
		{
			...
			sel_x = cols / 3;
			sel_y = rows / 3;
			stride = fe.layout.page_size = vert_flow ? cols :rows * 4;
		...
		function initial_load_setup( progress, var )
{
			...
			else
			{
				...
				if ( abs( var ) < cols)
				{
					...
					{
						snap.x = (((fe.layout.width /1.10 * ( progress * rows - r ) +  ((fe.layout.width - (fe.layout.width / 1.105448275)) / 2))- snap_width_delta) + PADX + 1)  ;
						snap.y = ((fe.layout.height / 1.51 + r * height + PADY)- snap_height_delta)+ PADY + 1;
					
					} else {
						snap.x = ((fe.layout.width /1.10 * ( progress * rows - r ) +  ((fe.layout.width - (fe.layout.width / 1.105448275)) / 2)) + PADX + 1);
