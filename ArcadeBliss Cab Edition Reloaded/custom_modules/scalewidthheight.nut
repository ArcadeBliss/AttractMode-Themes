/*
    ...
    USAGE
        ...
        animation.add( ScaleWidthHeight( object, config ) );
...
*/
class ScaleWidthHeight extends Animation
{
    version = 1.5;
    build = 100;
    object = null;
    ...
    constructor( object, config = null )
    {
        ...
        origin = { x = object.x, y = object.y, w = object.width, h = object.height, mx = object.x + ( object.width / 2 ), my = object.y + (object.height / 2) }
    }
    ...
    function onStart()
    {
        ...
        {
			case "scale-exact":
                if ( ostart == null ) config.start <- { w = object.width, h = object.height }; else config.start <- { w = evaluate(ostart.w, object.width), h = evaluate(ostart.h, object.height) };
                if ( oend == null ) config.end <- { w = object.width, h = object.height }; else config.end <- { w = evaluate(oend.w, object.width), h = evaluate(oend.h, object.height) };
                break;	
            default:
                if ( ostart == null ) config.start <- { w = object.width, h = object.height }; else config.start <- { w = evaluate(ostart.w, object.width), h = evaluate(ostart.h, object.height) };
                if ( oend == null ) config.end <- { w = object.width, h = object.height }; else config.end <- { w = evaluate(oend.w, object.width), h = evaluate(oend.h, object.height) };
    ...
    function onUpdate()
    {
        ...
        switch (config.property)
        {
			...
			case "scale":
                value = {
					w = calculate( config.start.w, config.end.w, time, config.time, config.tween, config.easing ),
					h = calculate( config.start.h, config.end.h, time, config.time, config.tween, config.easing )
				}
                transform_translate(origin.x, origin.y, value, object.rotation, "scale-exact");
                break;
            default:
    ...
    { 
		...
		local newHeight = (from = "scale") ? scale.h : scale ;
		local newWidth = (from = "scale") ? scale.w : scale ;
