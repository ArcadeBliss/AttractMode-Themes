...
local frame_width = ((width - 11 * PADX ) + (width * ENLARGE))+15;  // set the size of the frame larger than the snaps
local frame_height = ((height - 9 * PADY) + (height * ENLARGE))+15; // set the size of the frame larger than the snaps
local frameg_width = ((width - 11 * PADX ) + (width * ENLARGE))+15;  // set the size of the frame larger than the snaps
local frameg_height = ((height - 9 * PADY) + (height * ENLARGE))+15; // set the size of the frame larger than the snaps
			...
			else
			{
				...
				if ( abs( var ) < cols)
				{
					if (select_status == true)
					{
						...
						snap.y = ((fe.layout.height / 1.51 + r * height + PADY)- snap_height_delta)+ PADY;
				
					
					} else {
						snap.x = ((fe.layout.width /1.10 * ( progress * rows - r ) +  ((fe.layout.width - (fe.layout.width / 1.105448275)) / 2)) + PADX);
						snap.y = ((fe.layout.height / 1.51 + r * height + PADY)+ PADY );
					}
				}
			}
...
///////////////////////////////////////
	class ChangeArtwork
	{
		...
		constructor(trigger)
		{
			...
			{
				image.push(fe.add_artwork( i, 0, fe.layout.height / 20, fe.layout.width, fe.layout.height * 0.605));
...
//////////////////////////////////
   ...
   // Movement Animation for the bgforgrid
	...
	local logo2 = fe.add_image( "assets/UIelements/INFOBOX.png", fe.layout.width / 1.217 - 11, fe.layout.height / 4.245 , fe.layout.width /4.87142857143 , fe.layout.height /2.29007633588 );
	
// add the game manufacturer logo
	gridc.img_t = fe.add_image( "[!publisher]", fe.layout.width / 1.25 , fe.layout.height / 17, fe.layout.height / 2.91 - 40, fe.layout.height / 6.38 );
...
// add MenuControls artwork
	local menucontrols = fe.add_image("assets/uielements/Menucontrols.png",(fe.layout.width * ( -0.0240416666 ) - 10), fe.layout.height * 0.25566666666, fe.layout.width * 0.234375 + 10 , 145);
...
///////////////////////////////////////
	...
	local overlayMenuSur = overlaySurface.add_surface(372,240);
	overlayMenuSur.set_pos(134,120);
	
	// fade out the background
	overlayMenuSur.add_image("assets/uielements/menuframe.png",0,0,372,240); // Add the menu frame
	
	// create the listbox for the menu including text colors and styles
	local overlay_lb = overlayMenuSur.add_listbox(fe.layout.width / 640 + 110,fe.layout.height / 4.36 + 75,fe.layout.width / 4.42396, fe.layout.height / 13.5 - 35 ); //Add the listbox
	...
	gridc.over_t= overlaySurface.add_artwork( "wheel",fe.layout.width / 2.56 - 33, fe.layout.height / 2.5116-25, 208,57); //add the bartop picutre
