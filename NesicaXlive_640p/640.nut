...
const PADX=1; // Snap padding
const PADY=1; // Snap padding
...
local frame_width = ((width - 16 * PADX ) + (width * ENLARGE))+15;  // set the size of the frame larger than the snaps
local frame_height = ((height - 16 * PADY) + (height * ENLARGE))+15; // set the size of the frame larger than the snaps
			...
			animation.add( PropertyAnimation( frame, 
				{   
					...
					tween = Tween.Linear, 
						...
						x = (width * sel_x + ((fe.layout.width - (fe.layout.width / 1.105448275)) / 2)) - ((frame_width - (width))/2) -1, 
						y = ((fe.layout.height / 1.51 + height * sel_y)) - ((frame_height - (height))/2) + 1
					}, 
					time = transition_ms -75 
				} ) );
	...
	class MySlot extends ConveyorSlot
	{
		...
		function initial_load_setup( progress, var )
		{
			if ( vert_flow )
			{
				...
				if ( abs( var ) < rows )
				{
					snap.x =((width * sel_x)+ PADX + 1);
					snap.y =((fe.layout.height / 24 + ( fe.layout.height * 11 / 12 ) * ( progress * cols - c ) + PADY)+ PADY + 1)
				}
				else
				{
					...
					snap.x =((( c + prog ) * width + PADX)+ PADX + 1);
					snap.y =((fe.layout.height / 24 + r * height + PADY)+ PADY + 1);
				...
				{	
					...
					{   
						...
						easing = Easing.In,
						start = {
							x =  ((1.0 * rand() / RAND_MAX) * (fe.layout.width*3 + 1)),
							y = ((1.0 * rand() / RAND_MAX) * (fe.layout.height*2 + 1))
						},
						end = { 
							x = ((fe.layout.width /1.10 * ( progress * rows - r ) +  ((fe.layout.width - (fe.layout.width / 1.105448275)) / 2))+ PADX),
							y = ((fe.layout.height / 1.51 + r * height + PADY)+ PADY + 1)
						}, 
		...
		function on_progress( progress, var )
		{
			if ( vert_flow )
			{
				...
				{
					snap.x =(width * sel_x);
					snap.y =(fe.layout.height / 24 + ( fe.layout.height * 11 / 12 ) * ( progress * cols - c ) + PADX);
				}
				else
				{
					local prog = ::gridc.transition_progress;
					...
					snap.x =(( c + prog ) * width + PADX);
					snap.y =(fe.layout.height / 24 + r * height + PADY);
...
///////////////////////////////////////
	class ChangeArtwork
	{
		...
		constructor(trigger)
		{
			...
			{
				image.push(fe.add_artwork( i, 0, fe.layout.height / 20, fe.layout.width, fe.layout.height * 0.605 - 1));
...
//////////////////////////////////
	...
	local arrows = fe.add_image("assets/uielements/arrow.png", -1 ,fe.layout.height * 0.765 + 5 ,fe.layout.height / 15 ,fe.layout.height / 14);
	local arrow2 = fe.add_image("assets/uielements/arrow2.png", fe.layout.width / 1.048034 - 1 ,fe.layout.height * 0.765 + 5 ,fe.layout.height / 15,fe.layout.height / 14);
	...
	gridc.mfa_t = fe.add_artwork(my_config["logo"],  fe.layout.height / 72 - 5, fe.layout.height / 17, fe.layout.height / 2.91 - 40, fe.layout.height / 6.38);;
	gridc.mfa_t.preserve_aspect_ratio = true;
	gridc.mfa_t.trigger = Transition.EndNavigation;

	// add the infobox graphic
	local logo2 = fe.add_image( "assets/UIelements/INFOBOX.png", fe.layout.width / 1.217 - 10, fe.layout.height / 4.245 , 175 , 262 );
	
	// add the game manufacturer logo
	gridc.img_t = fe.add_image( "[!publisher]", fe.layout.width / 1.25 , fe.layout.height / 17, fe.layout.height / 2.91 - 50, fe.layout.height / 6.38 - 5);
   ...
   gridc.num_t.align = Align.Right;
	...
	local menucontrols = fe.add_image("assets/uielements/Menucontrols.png",(fe.layout.width * ( -0.0260416666 ) - 10), fe.layout.height * 0.29166666666 + 20, fe.layout.width * 0.234375 + 10 , 165);
...
///////////////////////////////////////
	...
	local overlayMenuSur = overlaySurface.add_surface(440,420);
	overlayMenuSur.set_pos(fe.layout.width / 2.5945 , fe.layout.height / 3.2727);
	
	// fade out the background
	overlayMenuSur.add_image("assets/uielements/menuframe.png",0,0,fe.layout.width / 4.363636,fe.layout.height /2.5714 ); // Add the menu frame
	
	// create the listbox for the menu including text colors and styles
	local overlay_lb = overlayMenuSur.add_listbox(fe.layout.width / 640,fe.layout.height / 4.36,fe.layout.width / 4.42396, fe.layout.height / 13.5); //Add the listbox
	...
	gridc.over_t= overlaySurface.add_artwork( my_config["logo"] ,fe.layout.width / 2.56, fe.layout.height / 2.5116, fe.layout.width / 4.5714 ,fe.layout.height / 8); //add the bartop picutre
	gridc.over_t.preserve_aspect_ratio = true;
